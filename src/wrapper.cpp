...
NMPCWrapper::NMPCWrapper() {
  acados_ocp_capsule = planner_payload_acados_create_capsule();

  new_time_steps = NULL;
  status = planner_payload_acados_create_with_discretization(acados_ocp_capsule,
                                                             N, new_time_steps);
  ...
  nlp_config = planner_payload_acados_get_nlp_config(acados_ocp_capsule);
  nlp_dims = planner_payload_acados_get_nlp_dims(acados_ocp_capsule);
  nlp_in = planner_payload_acados_get_nlp_in(acados_ocp_capsule);
  nlp_out = planner_payload_acados_get_nlp_out(acados_ocp_capsule);
  nlp_solver = planner_payload_acados_get_nlp_solver(acados_ocp_capsule);
  nlp_opts = planner_payload_acados_get_nlp_opts(acados_ocp_capsule);
  updateHoverInput();
...
bool NMPCWrapper::update(
  ...
  for (int i = 0; i < N; i++) {
    planner_payload_acados_update_params_sparse(
        acados_ocp_capsule, i, y_indeces, acados_in.yref + i * yRefSize,
        yRefSize);
  }
  planner_payload_acados_update_params_sparse(acados_ocp_capsule, N, y_indeces,
                                              acados_in.yref_e, yRefSize);

  // solve NMPC optimization
  acados_status = planner_payload_acados_solve(acados_ocp_capsule);
...
void NMPCWrapper::setWeightMatrices(std::vector<double> Q,
  ...
  if (params_size != expected_cost_params) {
    std::cerr << "[NMPCWrapper] setWeightMatrices size mismatch: got "
              ...
              << ", yRefSize=" << yRefSize << ")." << std::endl;
    return;
  }

  // send parameters to acados
  std::vector<int> params_indices(params_size);
  std::iota(params_indices.begin(), params_indices.end(), yRefSize);
  for (int i = 0; i < N; i++) {
    const int status = planner_payload_acados_update_params_sparse(
  ...
  // read back stage-0 parameters and verify the updated tail segment.
  std::vector<double> p_stage0(NP, 0.0);
  ocp_nlp_in_get(nlp_config, nlp_dims, nlp_in, 0, "p", p_stage0.data());
  const double *updated_segment = p_stage0.data() + yRefSize;
